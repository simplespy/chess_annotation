...
def get_model_pricing(provider, model):
    if provider == "openai":
        return OPENAI_PRICING_PER_MILLION.get(model)
    return None


def empty_usage(provider=None, model=None):
...
def usage_from_openai_response(resp, model):
    ...
    if pricing is not None:
        cost_usd = (prompt_tokens / 1_000_000) * pricing["input"] + (completion_tokens / 1_000_000) * pricing["output"]
...
def add_usage(total_usage, new_usage):
    total_usage["rounds"] += new_usage.get("rounds", 0)
    ...
    total_usage["cost_usd"] += new_usage.get("cost_usd", 0.0)
    if total_usage.get("pricing") is None and new_usage.get("pricing") is not None:
        total_usage["pricing"] = new_usage["pricing"]
    return total_usage
...
def call_openai(messages, tools=None, model="gpt-4o", temperature=0.3):
    ...
    usage = usage_from_openai_response(resp, model)
    return LLMResponse(text=text, tool_calls=tc), msg, usage
...
def call_anthropic(messages, tools=None, model="claude-sonnet-4-5-20250929",
    ...
    usage = empty_usage(provider="anthropic", model=model)
    return LLMResponse(text=text, tool_calls=tc), resp, usage


def call_llm(messages, tools=None, provider="openai", model=None, temperature=0.3):
    """Unified LLM call. Returns (LLMResponse, raw_response, usage)."""
    if provider == "openai":
        m = model or "gpt-4o"
...
def generate_with_tools(entry, engine_lines, DEMOS, provider=PROVIDER, model=MODEL):
    ...
    tool_log = []
    usage_total = empty_usage(provider=provider, model=model)

    for round_num in range(MAX_TOOL_ROUNDS):
        resp, raw, usage = call_llm(messages, tools=tool_schemas, provider=provider,
                                    model=model, temperature=0.3)
        add_usage(usage_total, usage)

        if not resp.tool_calls:
            return resp.text.strip(), tool_log, usage_total
    ...
    return resp.text.strip() if resp.text else "[max tool rounds reached]", tool_log, usage_total
...
if __name__ == "__main__":
    ...
    n_generated = 0
    n_skipped = 0
    total_usage = empty_usage(provider=PROVIDER, model=MODEL)
    ...
    for idx in range(START - 1, _end):
        ...
        # LLM extraction
        generated_text, tool_log, usage = generate_with_tools(entry, engine_lines, demos)
